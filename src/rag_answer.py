...
def make_clickable_source(source_document: str, page_number: int) -> str:
    # This works well in many PDF viewers (including browser PDF viewers)
...
def main():
    ...
    if not question:
        # default demo question (you can change it)
        question = "What does the report say about life expectancy?"
    ...
    for n in nodes:
        ...
        page = int(meta.get("page_number", -1))
        key = (src, page)
        if key not in seen and page != -1 and src != "unknown":
            seen.add(key)
